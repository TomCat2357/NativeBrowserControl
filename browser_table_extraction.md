...
### 1. ブラウザウィンドウの取得と接続
特定のタイトルを含むEdgeブラウザウィンドウを検索し、接続します。

```python
# ブラウザウィンドウ一覧を取得
list_browser_windows(browser="edge", require_visible=True)

# タイトルに特定文字列を含むウィンドウに接続
connect_browser(browser="edge", window_index=0)
```

### 2. Static/Button????????????
??????????????`filter_elements` ? `Static`/`Button` ???????????

```python
# ????????????
scan_elements(browser="edge")

# ?????????
filter_elements(browser="edge", class_names=["Static", "Button"])
```
