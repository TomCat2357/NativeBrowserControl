...
1. **要素スキャン**: `chrome_scan_elements` を実行して件数を確認
2. **一覧取得**: `chrome_list_elements` でダイアログ内の要素一覧を取得
3. **Edit要素を特定**: 一覧から「ファイル名(N):」に対応する `<Edit>` 要素のインデックスを確認
   - 通常は `[56] <Edit> ファイル名(N):` のような形式で表示される
4. **テキスト設定**: `chrome_set_element_text` でインデックスとファイルパスを指定
   - 例: `chrome_set_element_text(index=56, text="C:\\Users\\username\\Downloads\\file.txt")`
5. **開くボタンクリック**: `chrome_click_element` で「開く(O)」ボタンをクリック
...
2. `chrome_list_elements` で要素一覧を取得
3. 「ファイルをアップロード」ボタンを探す
   - 例: `[168] <Button> ファイルをアップロード. ドキュメント、データ、コードファイル`
4. `chrome_click_element` でそのボタンをクリック
5. `chrome_wait` で2秒程度待機（ダイアログが開くまで）
...
3. 「ファイル名(N):」の `<Edit>` 要素のインデックスを確認
   - 例: `[56] <Edit> ファイル名(N):`
4. `chrome_set_element_text` でファイルパスを入力
...
2. chrome_list_elements()
3. chrome_click_element(index=168)  # ファイルをアップロードボタン
4. chrome_wait(seconds=2)
5. chrome_scan_elements()
6. chrome_list_elements()
7. chrome_set_element_text(index=56, text="C:\\Users\\gk3t-\\Downloads\\a.txt")
8. chrome_click_element(index=61)  # 開く(O)ボタン
9. chrome_wait(seconds=2)
10. chrome_screenshot()  # 確認
```
